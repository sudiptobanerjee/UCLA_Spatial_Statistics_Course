...
nimbleInits <- list(
		    list(beta=rep(0, times=p), deltasq=1.0, sigmasqSp=1.0, phi=3/300,  yFit=rep(0, times=N), w=rep(0, times=N)),
		    list(beta=rep(0, times=p), deltasq=1.0, sigmasqSp=1.0, phi=3/300,  yFit=rep(0, times=N), w=rep(0, times=N)), 
		    list(beta=rep(0, times=p), deltasq=1.0, sigmasqSp=1.0, phi=3/300,  yFit=rep(0, times=N), w=rep(0, times=N))
)

rModel <- nimbleModel(code=geostatCodeFull, name="coloradoSP", constants=nimbleConstants, data=nimbleData)
...
samps <- runMCMC(cMCMC, inits=nimbleInits, nchains=n_chains, niter = n_iter, nburnin = n_burn, samplesAsCodaMCMC = TRUE)
...
CI_deltasq <- credibleIntervals[grep("deltasq", rownames(credibleIntervals)),]
CI_deltasq
